...
def load_config(path) -> LockFixConfig:
    ...
    veeam = VeeamConfig(
        ...
        auto_discover=bool_value(veeam_raw.get("auto_discover", True), True),
        discovery_candidates=[str(item).rstrip("/") for item in veeam_raw.get("discovery_candidates", [])],
        discovery_scan_local_subnet=bool_value(veeam_raw.get("discovery_scan_local_subnet", True), True),
