...
class LockFixTests(unittest.TestCase):
    ...
    def test_monitoring_chart_uses_compact_height(self) -> None:
        ...
        self.assertIn('id="monitoringChart" viewBox="0 0 920 210"', html)
        self.assertIn('class="date-range monitoring-date-range"', html)
        self.assertIn("const height = 210;", app)
        self.assertIn("const pad = { left: 90, right: 22, top: 6, bottom: 28 };", app)
        self.assertIn("min-height: 172px;", css)
        self.assertIn("font-size: 9.4px;", css)
        self.assertIn("font-size: 9.6px;", css)
    ...
    def test_report_export_buttons_use_file_icons_and_pdf(self) -> None:
        ...
        self.assertIn('class="file-icon file-excel"', report_header)
        ...
        self.assertNotIn('href="/api/report.hwpx"', report_header)
        self.assertIn('data-i18n="report.exportExcel"', report_header)
        self.assertIn('"report.exportExcel": "Excel"', app)
        self.assertIn('"report.exportExcel": "엑셀"', app)
        self.assertIn('"report.exportPdf": "PDF"', app)
        self.assertIn("function updateReportExportLinks", app)
        self.assertIn('[".report-export-excel", "/api/report.xlsx"]', app)
        ...
        self.assertIn('elif parsed.path in {"/api/report.hwp", "/api/report.hwpx"}:', webui_source)
        self.assertIn("def send_report_hwp", webui_source)
        self.assertIn("application/hwp+zip", webui_source)
        self.assertIn("def build_hwpx_report", webui_source)
        ...
        self.assertIn("Resource Usage Analysis", webui_source)
        ...
        self.assertIn("20260529-report-card-spacing", html)
    ...
    def test_report_exports_are_balanced_documents(self) -> None:
        ...
        self.assertIn(b"Pending", pdf_body)
        ...
        self.assertEqual(captured_hwp["filename"], "lockfix_report.hwpx")
        self.assertIn("application/hwp+zip", captured_hwp["content_type"])
        self.assertTrue(zipfile.is_zipfile(webui.io.BytesIO(captured_hwp["body"])))
        with zipfile.ZipFile(webui.io.BytesIO(captured_hwp["body"])) as archive:
            ...
            self.assertIn("Settings/settings.xml", names)
            self.assertIn("Preview/PrvText.txt", names)
            self.assertEqual(zipfile.ZIP_STORED, archive.getinfo("mimetype").compress_type)
            self.assertEqual("application/hwp+zip", archive.read("mimetype").decode("utf-8"))
            content_hpf = archive.read("Contents/content.hpf").decode("utf-8")
            self.assertIn('href="Contents/header.xml"', content_hpf)
            self.assertIn('href="Contents/section0.xml"', content_hpf)
            self.assertIn("Contents/section0.xml", archive.read("META-INF/manifest.xml").decode("utf-8"))
            section_xml = archive.read("Contents/section0.xml").decode("utf-8")
            self.assertIn("<hp:secPr", section_xml)
            self.assertIn("<hp:colPr", section_xml)
            self.assertIn("LOCK-FIX System Inspection Report", section_xml)
            self.assertIn("Customer / Inspection Information", section_xml)
            self.assertIn("Server Basic Information", section_xml)
            self.assertIn("Resource Usage Analysis", section_xml)
            self.assertIn("Server Inspection Checklist", section_xml)
            self.assertIn("Signature Confirmation", section_xml)
            self.assertIn("Attached", section_xml)
            self.assertNotIn("Recent Monitoring Samples", section_xml)
            self.assertIn("OAM Electronics Co., Ltd.", section_xml)
        ...
        handler.path = "/api/report.hwpx?lang=ko"
        captured_korean_hwp = {}
        handler.send_download = lambda body, content_type, filename: captured_korean_hwp.update(
            {"body": body, "content_type": content_type, "filename": filename}
        )
        handler.send_report_hwp()
        with zipfile.ZipFile(webui.io.BytesIO(captured_korean_hwp["body"])) as archive:
            korean_section_xml = archive.read("Contents/section0.xml").decode("utf-8")
            self.assertIn("LOCK-FIX 시스템 점검 보고서", korean_section_xml)
            self.assertIn("고객 / 점검 정보", korean_section_xml)
            self.assertIn("서버 점검 체크리스트", korean_section_xml)
            self.assertIn("전자 서명", korean_section_xml)
            self.assertNotIn("Recent Monitoring Samples", korean_section_xml)
    ...
    def test_report_inspection_result_badges_are_centered(self) -> None:
        ...
        self.assertIn("padding: 4px 22px 22px;", css)
        self.assertIn(".report-usage-panel + .report-inspection-panel", css)
    ...
    def test_user_management_ui_has_i18n_actions_and_cache_bust(self) -> None:
        ...
        for token in [
            ...
            'v=20260522-airgap-rest-live-stream',
    ...
    def test_detect_webui_uses_judgement_module_layout(self) -> None:
        ...
        self.assertGreaterEqual(app_source.count('data-detect-temperature-risk-card="true"'), 2)
        ...
        self.assertIn("20260531-temperature-risk-fallback", index_source)
    ...
    def test_webui_sidebar_is_compact_to_prioritize_content(self) -> None:
        ...
        self.assertIn("padding: 18px 11px;", css_source)
        self.assertIn("column-gap: 9px;", css_source)
        self.assertIn("min-width: 62px;", css_source)
    ...
    def test_sidebar_user_menu_shows_current_session_without_navigation(self) -> None:
        ...
        self.assertIn("20260522-airgap-rest-live-stream", index_source)
    ...
    def test_ops_events_panel_is_hidden_while_background_collection_remains(self) -> None:
        ...
        self.assertIn("v=20260522-airgap-rest-live-stream", html_source)
    ...
    def test_network_detail_cards_use_subtle_show_toggles(self) -> None:
        ...
        self.assertIn("20260522-airgap-rest-live-stream", html_source)
    ...
    def test_settings_view_uses_full_width_balanced_grid(self) -> None:
        ...
        self.assertIn("20260529-settings-field-contain", html_source)
        self.assertIn("grid-template-columns: minmax(0, 0.72fr) minmax(0, 1.28fr);", css_source)
        self.assertIn("grid-template-columns: minmax(0, 0.7fr) minmax(0, 1.3fr);", css_source)
        self.assertIn(".settings-card > div,", css_source)
        self.assertIn(".settings-card label,", css_source)
    ...
    def test_dashboard_cards_drag_resize_without_edit_button(self) -> None:
        ...
        self.assertIn('const dashboardPanelOrderKey = "lockfix.dashboard.panelOrder.v3";', app_source)
        self.assertIn('const dashboardPanelSizeKey = "lockfix.dashboard.panelSize.v5";', app_source)
        self.assertIn('const backupJobsColumnSizeKey = "lockfix.dashboard.backupJobsColumnSize.v1";', app_source)
        self.assertIn('data-dashboard-panel="backupJobs" data-panel-fixed="true" data-cols="12" data-rows="2"', app_source)
        self.assertLess(app_source.index('data-dashboard-panel="backupJobs"'), app_source.index('data-dashboard-panel="protection"'))
        self.assertIn('data-dashboard-panel="backup" data-panel-resizable="true" data-cols="12" data-rows="3"', app_source)
        self.assertIn(".backup-jobs-panel {\n  min-height: 236px;", css_source)
        self.assertIn("table-layout: fixed;", css_source)
        self.assertIn("min-width: 1320px;", css_source)
        self.assertIn("min-height: 58px;", css_source)
        self.assertIn("border-bottom: 1px solid #d8e3ee;", css_source)
        self.assertIn("margin-top: 10px;", css_source)
        self.assertIn("padding: 8px 10px;", css_source)
        self.assertIn("border-right: 1px solid #e4edf6;", css_source)
        self.assertIn("width: 260px;", css_source)
        self.assertIn("width: 116px;", css_source)
        self.assertIn("text-align: left;", css_source)
        self.assertIn("padding-right: 10px;", css_source)
        self.assertIn("width: 410px;", css_source)
        self.assertIn("td.backup-job-name", css_source)
        self.assertIn("td.backup-job-description", css_source)
        self.assertIn("text-overflow: clip;", css_source)
        self.assertIn("backup-job-col-resizer", css_source)
        self.assertIn("cursor: col-resize;", css_source)
        self.assertIn("function enableBackupJobsColumnResize()", app_source)
        self.assertIn("backupJobColgroupHtml(backupJobColumnWidths)", app_source)
        self.assertIn("backupJobHeaderCell(jobColumns.name || \"Name\", 0)", app_source)
        self.assertIn("saveBackupJobColumnWidths(widths)", app_source)
        self.assertIn("data-resizable-table=\"veeam-jobs\"", app_source)
        self.assertIn('class="backup-job-name"', app_source)
        ...
        self.assertIn('data-dashboard-panel="backup" data-panel-resizable="true" data-cols="12" data-rows="3"', app_source)
        self.assertIn('data-panel-fixed="true"', app_source)
        ...
        self.assertIn("--flow-gap: clamp(16px, 2.8vw, 54px);", css_source)
        self.assertIn("grid-template-columns: repeat(5, minmax(0, 1fr));", css_source)
        ...
        self.assertIn("20260522-airgap-rest-live-stream", html_source)
        self.assertIn("justify-content: stretch;", css_source)
        self.assertIn("padding: 0 clamp(18px, 2vw, 30px);", css_source)
    ...
    def test_airgap_procedure_steps_show_live_motion_state(self) -> None:
        ...
        self.assertIn("function renderSources", app_source)
        self.assertIn("const stepLiveState = (item) =>", app_source)
        self.assertIn("const stepLiveClass = (item) => `veeam-step-live-${stepLiveState(item)}`;", app_source)
        self.assertIn('data-step-state="${stepLiveState(item)}"', app_source)
        self.assertIn(".airgap-procedure-steps .veeam-step-live-running", css_source)
        self.assertIn("animation: airgapStepLivePulse 0.92s ease-in-out infinite;", css_source)
    ...
    def test_airgap_procedure_table_includes_live_action_row(self) -> None:
        ...
        self.assertIn("const airGapLogTime = (item) =>", app_source)
        self.assertIn("const actions = stepLogs.map((log, index) =>", app_source)
        self.assertIn("Veeam REST API synchronization", app_source)
        self.assertIn('name: "Veeam REST API"', app_source)
        self.assertIn("Step colors and arrows stay fixed until a real Veeam REST API session is received.", app_source)
        self.assertNotIn("<th>Status</th>", app_source)
        self.assertNotIn('<col class="veeam-log-col-status" />', app_source)
        self.assertNotIn("veeam-session-status veeam-session-", app_source)
