...
def create_veeam_client(veeam_config: dict[str, Any]) -> VeeamClient:
    ...
    username = str(veeam_config.get("username") or os.getenv(username_env) or "").strip()
    ...
    settings = VeeamSettings(
        ...
        job_name=str(veeam_config.get("job_name") or ""),
        job_id=normalized_job_id(str(veeam_config.get("job_id") or "")),
        require_backup_copy=bool_value(veeam_config.get("require_backup_copy", True), True),
        target_repository_id=str(veeam_config.get("target_repository_id") or ""),
        target_repository_name=str(veeam_config.get("target_repository_name") or ""),
        target_repository_path=str(veeam_config.get("target_repository_path") or ""),
